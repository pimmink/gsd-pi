...
| GSD-W014 | GitHub Copilot model-catalog sync (`/gsd copilot-models`) | Upstream | PR required | — | — | `feat/github-copilot-model-catalog-sync` | Complete — exact-sha **sharded** remote verification passed on SHA `26ae1e55d972781db4dbaf475f566fe3d746affe` with build + lifecycle-shadow gate + all 4 test shards green (run `32203423874`); supersedes the earlier stable-only run `32199524656` at `4bdb8909`. | Phase K is now genuinely complete (strict why parser, no default-model fallback, full effective-local/session-available/live-catalog/economics/routing-eligibility contract); prepare the upstream PR per CONTRIBUTING.md. |
