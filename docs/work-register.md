...
| GSD-W029 | Truthful GitHub Copilot catalog normalization and bounded diagnostics | Upstream | PR required | [#2088](https://github.com/open-gsd/gsd-pi/issues/2088) | [#2092](https://github.com/open-gsd/gsd-pi/pull/2092) | `fix/copilot-catalog-truthful-normalization` | Complete / PR updated — `954fb042` pushed; placeholder completions remain removed, real model-ID completion for `pricing`/`why` is covered, unknown metadata stays unknown, provider-live/static transport conflicts are recorded, and authoritative input modality is required before overlay synthesis. Focused suites 95/95 and 65/65 green; extension typecheck and diff check green; Copilot review threads resolved | Wait for fresh upstream CI and maintainer review on PR #2092 |
...
| GSD-W032 | Route bundled Copilot Kimi, Gemini 3.x, Grok 4.6, MAI Flash Picker, and GPT-5.4 Nano models | Upstream | PR required | — | — | `fix/copilot-router-model-coverage` | In progress / remote CI rerun — `85dd3513` pushed to origin; router now covers the remaining bundled Copilot chat IDs missing tier/profile rows, and Copilot Kimi catalog transport is covered. Local pi-ai/router/typecheck/verify:fast checks are green. Failed sharded run 33656066924 exposed a gsd-pi-ci artifact bug; fixed in gsd-pi-ci `ede043a` and redispatched run 33664264464 | Wait for remote sharded CI run 33664264464 before local installation or upstream PR |
