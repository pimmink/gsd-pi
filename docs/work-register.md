...
| GSD-W031 | Safe account-scoped GitHub Copilot model suggestions | Upstream | PR required | [#2088](https://github.com/open-gsd/gsd-pi/issues/2088) | [#2093](https://github.com/open-gsd/gsd-pi/pull/2093) | `fix/copilot-suggestion-safety` | Complete / PR updated — `00f9ee0f` pushed after rebase to current `upstream/main`; dominance now fails closed on missing/non-finite dimensions, resolves inherited profiles through the router resolver, preserves unknown tool-call diagnostics, and covers the three-model ordering regression. Focused suites 204/204 green; extension typecheck, diff check, and upstream CI run 33650154545 green; Copilot review threads resolved | Wait for maintainer review on PR #2093 |
