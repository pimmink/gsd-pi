...
canonical source. GitHub and local Git evidence was refreshed on **2026-08-25**.
...
| GSD-W021 | Repair evidence-backed lifecycle shadow authority before Milestone validation | Upstream | PR required | — | — | `fix/lifecycle-shadow-authority-repair` | Complete — `bb75b2f9`; 20/20 focused lifecycle tests, extension typecheck, core build, and `verify:fast` passed with frozen lockfile. Partial-adoption repair is evidence-gated, task-before-slice, replay-safe, and runs before the fresh validation receipt; strict completion guards remain unchanged. | Open upstream PR |
