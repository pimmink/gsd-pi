...
| GSD-W014 | GitHub Copilot model-catalog sync (`/gsd copilot-models`) | Upstream | PR required | — | — | `feat/github-copilot-model-catalog-sync` | Complete — rebased a second time to `fa4672fc4dd2609a35b232d7d47b5f63f35e173d`. Run `32646186707` had `verify-pr` succeed but `verify-merge` fail on 2 tests in an unrelated pre-existing file (`tool-surface-readiness.test.ts`, an MCP tool-registration timing test never touched by this feature) — confirmed non-reproducible in 3 local isolated runs (23/23 pass each time), diagnosed as a transient flake. Redispatched the same SHA as run `32648668119`, which came back fully GREEN (`verify-pr` + `verify-merge` both success). This is the submission-grade exact-SHA evidence. | Refresh local `.plans/` drafts to cite run `32648668119`, then dispatch GSD-W018's own rebased-SHA full-gate next |
...
| GSD-W017 | Cheaper same-tier Copilot suggestions in `pricing`/`why`, plus proactive notifications | Upstream | PR required | — | — | `feat/copilot-cheaper-model-suggestions` | Complete — rebased a second time to `007e3c2c65a8f1390bb74f1a2099a9befa7f400e` onto GSD-W018's re-rebased tip; `git range-diff` confirmed content-identical, 81/81 tests and `typecheck:extensions` green. Exact-SHA full-gate run `32652579050` came back fully GREEN (`verify-pr` + `verify-merge` both success). This is the submission-grade exact-SHA evidence. Prior in-flight run `32645096655` on now-superseded head `6530030c...` is historical only. | All three branches (W014, W018, W017) now green on final SHAs — proceed to final consolidated report |
| GSD-W018 | Session-start GitHub Copilot catalog refresh and runtime model activation | Upstream | PR required | — | — | `feat/copilot-catalog-session-refresh` | Complete — rebased a second time to `d7acc03486b2a549b69d9bb35206b34988ff5ce0` onto GSD-W014's re-rebased tip; `git range-diff` confirmed both commits content-identical, 32/32 tests and `typecheck:extensions` green. Exact-SHA full-gate run `32650707615` came back fully GREEN (`verify-pr` + `verify-merge` both success). This is the submission-grade exact-SHA evidence. Prior green run `32485730290` on now-superseded head `7961ebfa...` remains historical evidence. | Local `.plans/` drafts refreshed to cite run `32650707615`; GSD-W017's own rebased-SHA full-gate now dispatched (run `32652579050`) |
