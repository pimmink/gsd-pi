...
| GSD-W029 | M011 Milestone re-run plan (post lifecycle-shadow-repair) | Fork-local | No PR planned | — | — | `docs/m011-resumption-plan` | Idea — local branch created from `upstream/main` (`327343af`); no code yet | Wait for GSD-W021 to merge/activate, then backup DB+WAL+SHM, verify hashes, fresh session, one new validation |
