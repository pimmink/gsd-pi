...
| GSD-W014 | GitHub Copilot model-catalog sync (`/gsd copilot-models`) | Upstream | PR required | — | — | `feat/github-copilot-model-catalog-sync` | In progress — rebased a second time to `fa4672fc4dd2609a35b232d7d47b5f63f35e173d`; run `32646186707` had `verify-pr` succeed but `verify-merge` fail on 2 tests in an unrelated pre-existing file (`tool-surface-readiness.test.ts`, an MCP tool-registration timing test never touched by this feature) — confirmed non-reproducible in 3 local isolated runs (23/23 pass each time), diagnosed as a transient flake, redispatched the same SHA as run `32648668119` (pending). | Await run `32648668119`; if green, dispatch GSD-W018's own rebased-SHA full-gate next |
