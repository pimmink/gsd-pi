...
`auto-model-selection.ts`. Requested a rebase; PR #1978 unblocks the stacked #1979/#1980
once rebased. GSD-W014/W017/W018 marked `Blocked` below pending an authorized rebase pass.
...
| GSD-W014 | GitHub Copilot model-catalog sync (`/gsd copilot-models`) | Upstream | PR required | [#1978](https://github.com/open-gsd/gsd-pi/pull/1978) | — | `feat/github-copilot-model-catalog-sync` | **Blocked** — 2026-08-28: maintainer `jeremymcs` flagged that PR #1978 now conflicts with merged upstream PR [#2035](https://github.com/open-gsd/gsd-pi/pull/2035) (dotted-aware `canonicalModelId` normalization in `model-router.ts`, touching the same `auto-model-selection.ts` surface this stack modifies) and requested a rebase; rebasing here unblocks the stacked #1979/#1980. Prior state: rebased a fourth time to `9c84c780a13f0a1ac1036600601a8a3dbdd33002`; exact-SHA full-gate run `32663412416` came back fully GREEN. 2026-08-24: addressed all 14 GitHub Copilot automated-review findings (12 posted + 2 overview-only) in commit `cf4d2c13`, then two independent `codex review` passes caught 6 more real issues across 2 correction rounds (economics precedence, account-key collisions, an implicit cost fallback that kept applying internally, a routing fix overriding an explicit `tier_models` pin, and a genuine `models.json` override still losing to live pricing) — all fixed across commits `16baf772` and `1439c2d0`. `tsc --noEmit` clean; 283+ tests green across 9 files. Exact-SHA remote gate `32743896391` green for the intermediate `16baf772`. 2026-08-24: fixed a batch of cosmetic markdownlint/biome findings in commit `c6e108ea` (pushed, was `1439c2d0`) — no functional changes. | Not yet rebased. Needs a fresh worktree rebase of `feat/github-copilot-model-catalog-sync` onto the post-#2035 upstream/main tip, conflict resolution reconciling W014's capability-profile-confidence routing with #2035's dotted-ID canonicalization, full local verification, then a cascade rebase of GSD-W018/GSD-W017 before pushing. Requires explicit go-ahead before starting. |
...
| GSD-W017 | Cheaper same-tier Copilot suggestions in `pricing`/`why`, plus proactive notifications | Upstream | PR required | [#1980](https://github.com/open-gsd/gsd-pi/pull/1980) | — | `feat/copilot-cheaper-model-suggestions` | **Blocked** — depends on GSD-W014's rebase (maintainer flagged a conflict with merged upstream PR #2035 on PR #1978; #1979/#1980 are stacked on #1978). Prior state: rebased a fourth time to `da4f2b4422530d8cca69c7b4d4e33937810b3197` onto GSD-W018's fourth-rebase tip; exact-SHA full-gate run `32670073187` came back fully GREEN. 2026-08-24: rebased a fifth time onto GSD-W018's `91787bcd`, resolving a real merge conflict in `copilot-models.ts` plus a silent signature-drift break caught by `tsc`, plus 2 confirmed pre-existing test-fixture bugs — all fixed in commit `2a936c03`. `tsc --noEmit` clean; 269/269 tests green across 9 files. | Once GSD-W014 and GSD-W018 have rebased onto the post-#2035 upstream/main tip, rebase this branch onto GSD-W018's new tip via `git rebase --onto`, re-verify, and force-push. Do not rebase out of order. |
| GSD-W018 | Session-start GitHub Copilot catalog refresh and runtime model activation | Upstream | PR required | [#1979](https://github.com/open-gsd/gsd-pi/pull/1979) | — | `feat/copilot-catalog-session-refresh` | **Blocked** — depends on GSD-W014's rebase (maintainer flagged a conflict with merged upstream PR #2035 on PR #1978, which this branch is stacked on). Prior state: rebased a fourth time to `886c4b41c3db3cb244de4ea6215c9f0771230b1d` onto GSD-W014's fourth-rebase tip; exact-SHA full-gate run `32666347203` came back fully GREEN. 2026-08-24: rebased a fifth time onto GSD-W014's `c6e108ea` (clean, zero conflicts) and fixed a cross-provider price-bleed bug in commit `91787bcd`. `tsc --noEmit` clean; 25/25 tests green. | Once GSD-W014 rebases onto the post-#2035 upstream/main tip, rebase this branch onto GSD-W014's new tip via `git rebase --onto`, re-verify, and force-push before GSD-W017 rebases onto this tip in turn. |
