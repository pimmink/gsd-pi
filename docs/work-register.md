...
| GSD-W018 | Session-start GitHub Copilot catalog refresh and runtime model activation | Upstream | PR required | — | — | `feat/copilot-catalog-session-refresh` | In progress — rebased a second time to `d7acc03486b2a549b69d9bb35206b34988ff5ce0` onto GSD-W014's re-rebased tip; `git range-diff` confirmed both commits content-identical, 32/32 tests and `typecheck:extensions` green. Exact-SHA full-gate not yet dispatched (waiting on GSD-W014 to confirm green first). Prior green run `32485730290` on now-superseded head `7961ebfa...` remains historical evidence. | Once GSD-W014 is confirmed green, dispatch this branch's own exact-SHA full-gate, then GSD-W017's |
