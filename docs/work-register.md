...
| GSD-W021 | Repair evidence-backed lifecycle shadow authority before Milestone validation | Upstream | PR required | — | [#2002](https://github.com/open-gsd/gsd-pi/pull/2002) | `fix/lifecycle-shadow-authority-repair` | PR open — `bb75b2f9`; 20/20 focused lifecycle tests, extension typecheck, core build, and `verify:fast` passed with frozen lockfile. Partial-adoption repair is evidence-gated, task-before-slice, replay-safe, and runs before the fresh validation receipt; strict completion guards remain unchanged. | Monitor PR #2002 for maintainer feedback and CI |
