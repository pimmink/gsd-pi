...
| GSD-W017 | Cheaper same-tier Copilot suggestions in `pricing`/`why`, plus proactive notifications | Upstream | PR required | — | — | `feat/copilot-cheaper-model-suggestions` | In progress — rebased a second time to `007e3c2c65a8f1390bb74f1a2099a9befa7f400e` onto GSD-W018's re-rebased tip; `git range-diff` confirmed content-identical, 81/81 tests and `typecheck:extensions` green. Exact-SHA full-gate not yet dispatched (waiting on GSD-W014 then GSD-W018 to confirm green first). Prior in-flight run `32645096655` on now-superseded head `6530030c...` is historical only. | Once GSD-W014 and GSD-W018 are both confirmed green, dispatch this branch's own exact-SHA full-gate |
| GSD-W018 | Session-start GitHub Copilot catalog refresh and runtime model activation | Upstream | PR required | — | — | `feat/copilot-catalog-session-refresh` | In progress — rebased a second time to `d7acc03486b2a549b69d9bb35206b34988ff5ce0` onto GSD-W014's re-rebased tip; `git range-diff` confirmed both commits content-identical, 32/32 tests and `typecheck:extensions` green. GSD-W014 confirmed green (run `32648668119`); exact-SHA full-gate for this head now dispatched as run `32650707615` (pending). Prior green run `32485730290` on now-superseded head `7961ebfa...` remains historical evidence. | Await run `32650707615`; if green, dispatch GSD-W017's own rebased-SHA full-gate next |
