...
| GSD-W022 | Agent-core resolved tool result `isError` dropped in the agent loop | Upstream | PR required | — | — | `fix/agent-core-tool-result-iserror` | **Blocked** — reproduction test added locally; current loop emits `isError=false` for a resolved `{isError:true}` result. Native MCP and external Claude adapters already preserve their error flags; the shared `pi-agent-core` path loses the native result-level signal. `AgentToolResult` currently documents throw-on-failure and does not declare `isError`, so the proposed fix adds a public error-signaling contract. | Obtain upstream RFC/maintainer approval for the `AgentToolResult.isError` contract or agree on an adapter-only alternative; then implement and run the verification ladder. No production code, issue, push, or PR until approval |
