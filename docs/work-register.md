...
| GSD-W017 | Cheaper same-tier Copilot suggestions in `pricing`/`why`, plus proactive notifications | Upstream | PR required | — | — | `feat/copilot-cheaper-model-suggestions` | In progress — rebuilt on the confirmed-green GSD-W018 tip; real notification feature implemented (manual-selection + post-refresh triggers, capability-aware messaging, fingerprint dedup). 81/81 tests pass, `typecheck:extensions` clean. Committed `6530030cd1a0b5cadc0f5222c35d2fc9f11d8422`, pushed, exact-SHA full-gate run `32645096655` dispatched (pending). | Await run `32645096655`; if green, consider feature-complete pending PR-readiness review |
| GSD-W018 | Session-start GitHub Copilot catalog refresh and runtime model activation | Upstream | PR required | — | — | `feat/copilot-catalog-session-refresh` | Complete — exact-SHA remote full-gate run `32485730290` on `7961ebfa23e3732f97586192458af8f35f3e565d` confirmed both `verify-pr (literal)` and `verify-merge (literal)` succeeded, after fixing 2 real gaps found by the first attempt (`32482885693`). | None further for W018 itself; GSD-W017 has been rebuilt on this SHA and needs its notification feature completed |
