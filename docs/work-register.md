...
| GSD-W021 | Repair evidence-backed lifecycle shadow authority before Milestone validation | Upstream | PR required | — | [#2002](https://github.com/open-gsd/gsd-pi/pull/2002) | `fix/lifecycle-shadow-authority-repair` | **PR open / Review fixes pushed** — `0713f16c` pushed to PR #2002 addressing all 4 Copilot review comments (Milestone state guard, deterministic batch keys, Task-before-Slice execution order). All 32 focused tests pass locally (100%). Fresh CI checks triggered | Await maintainer review/merge on PR #2002 |
| GSD-W022 | Agent-core resolved tool result `isError` dropped in the agent loop | Upstream | PR required | [#2015](https://github.com/open-gsd/gsd-pi/issues/2015) | [#2016](https://github.com/open-gsd/gsd-pi/pull/2016) | `fix/agent-core-tool-result-iserror`, `fix/w022-local-patch` | **Draft PR open / Local patch installed** — Draft PR #2016 open with reproduction test (`fd9946ca`). Fix implemented on local branch `fix/w022-local-patch` (`5bfcc551`, 29/29 tests pass). Installed local community extension patch `w022-iserror-patch` at `~/.gsd/agent/extensions/` using `tool_result` hook so `/gsd auto` in Edelman Studio preserves `isError` immediately | Await maintainer decision on RFC issue #2015 / Draft PR #2016; local patch active in `~/.gsd/agent/extensions/` for local execution |
