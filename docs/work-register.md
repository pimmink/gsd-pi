...
| GSD-W017 | Cheaper same-tier Copilot suggestions in `pricing`/`why`, plus proactive notifications | Upstream | PR required | [#1980](https://github.com/open-gsd/gsd-pi/pull/1980) | — | `feat/copilot-cheaper-model-suggestions` | Complete — 2026-08-28: rebased onto GSD-W018's new tip (0 behind, 3 ahead), clean (no conflicts). Fixed 2 more of the same dotted-ID registry bugs in this branch's own files: `findCheaperSameTierOption`'s tier lookups and `copilot-catalog-notifications.ts`'s `isHigherCapability` both indexed `MODEL_CAPABILITY_TIER`/`MODEL_CAPABILITY_PROFILES` with a raw dotted bare ID instead of `canonicalizeModelId`. `tsc --noEmit` clean; full 9-file W017 test surface 286/286 green (including the 7 that failed before the fix). Force-pushed `8d4a1e1d` to `origin/feat/copilot-cheaper-model-suggestions` (was `0d416566`). Prior state: rebased a fourth time to `da4f2b4422530d8cca69c7b4d4e33937810b3197` onto GSD-W018's fourth-rebase tip; exact-SHA full-gate run `32670073187` came back fully GREEN. 2026-08-24: fifth rebase onto GSD-W018's `91787bcd`, resolved a real merge conflict and 2 pre-existing test-fixture bugs, fixed in commit `2a936c03`. | Monitor PR #1980 for maintainer feedback. |
| GSD-W018 | Session-start GitHub Copilot catalog refresh and runtime model activation | Upstream | PR required | [#1979](https://github.com/open-gsd/gsd-pi/pull/1979) | — | `feat/copilot-catalog-session-refresh` | Complete — 2026-08-28: rebased onto GSD-W014's new post-#2035 tip (0 behind, 3 ahead). One real conflict in `copilot-models.ts` (import-only, combined both sides). Fixed the same dotted-ID class of bug in this branch's own `classifyRemoteCopilotModel` (commit `9e60b9c9`): its `MODEL_CAPABILITY_TIER` lookup was keyed on the raw live-record model ID instead of `canonicalizeModelId`. `tsc --noEmit` clean; `copilot-catalog-session-refresh.test.ts` + `model-router.test.ts` + `copilot-models-handler.test.ts` 208/208 green. Force-pushed to `origin/feat/copilot-catalog-session-refresh` (was `3f8a3583`). Prior state: rebased a fourth time to `886c4b41c3db3cb244de4ea6215c9f0771230b1d` onto GSD-W014's fourth-rebase tip; exact-SHA full-gate run `32666347203` came back fully GREEN. 2026-08-24: fifth rebase onto GSD-W014's `c6e108ea`, fixed a cross-provider price-bleed bug in commit `91787bcd`. | PR #1980 (GSD-W017) rebased onto this new tip. Monitor PR #1979 for maintainer feedback. |
