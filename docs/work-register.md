...
| GSD-W014 | GitHub Copilot model-catalog sync (`/gsd copilot-models`) | Upstream | PR required | — | — | `feat/github-copilot-model-catalog-sync` | In progress — upstream/main advanced a 3rd time (`b5ff8ced`→`60f6919`); rebased a third time to `4339e623c048f0289b717a38f9b01fa916358f44`, `git range-diff` confirmed all 18 commits content-identical, focused regression `209/209` and `typecheck:extensions` green. Exact-SHA full-gate run `32655349780` dispatched (pending). Prior green run `32648668119` on now-superseded head `fa4672fc...` remains historical evidence. | Await run `32655349780`; if green, dispatch GSD-W018's own third-rebase full-gate next |
...
| GSD-W017 | Cheaper same-tier Copilot suggestions in `pricing`/`why`, plus proactive notifications | Upstream | PR required | — | — | `feat/copilot-cheaper-model-suggestions` | In progress — rebased a third time to `bcc3946a28d9debf574a90b86f15d7872aa19101` onto GSD-W018's third-rebase tip; `git range-diff` confirmed content-identical, `typecheck:extensions` green (local dist-test run deferred to remote CI due to a known cross-worktree build-plumbing artifact, not a code issue). Exact-SHA full-gate not yet dispatched (waiting on GSD-W014 then GSD-W018 to confirm green first). Prior green run `32652579050` on now-superseded head `007e3c2c...` remains historical evidence. | Once GSD-W014 and GSD-W018 are both confirmed green, dispatch this branch's own exact-SHA full-gate |
| GSD-W018 | Session-start GitHub Copilot catalog refresh and runtime model activation | Upstream | PR required | — | — | `feat/copilot-catalog-session-refresh` | In progress — rebased a third time to `995518351238ddaf1b3d9273b51481b41ea74029` onto GSD-W014's third-rebase tip; `git range-diff` confirmed both commits content-identical, `typecheck:extensions` green (local dist-test run deferred to remote CI due to a known cross-worktree build-plumbing artifact, not a code issue). Exact-SHA full-gate not yet dispatched (waiting on GSD-W014 to confirm green first). Prior green run `32650707615` on now-superseded head `d7acc034...` remains historical evidence. | Once GSD-W014 is confirmed green, dispatch this branch's own exact-SHA full-gate, then GSD-W017's |
