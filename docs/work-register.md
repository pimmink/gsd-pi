...
| GSD-W017 | Cheaper same-tier Copilot suggestions in `pricing`/`why` | Upstream | PR required | — | — | `feat/copilot-cheaper-model-suggestions` | In progress — rebuilt on the confirmed-green GSD-W018 tip `7961ebfa23e3732f97586192458af8f35f3e565d`; focused handler/cost-table suites (69/69) and `typecheck:extensions` are green on the rebuilt worktree. No commit, push, or remote CI yet for this state. | Implement the real notification feature (manual-selection + post-refresh triggers, dedup, no auto-switch), add tests, then commit/push/dispatch exact-SHA full-gate |
