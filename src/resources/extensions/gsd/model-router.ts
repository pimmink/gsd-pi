...
export function getModelProfileConfidence(
	...
	const override =
		capabilityOverrides?.[modelId] ?? capabilityOverrides?.[bareId];
	if (
		override &&
		CAPABILITY_DIMENSIONS.every((d) => typeof override[d] === "number")
	) {
		return "provisional";
	}
	return "unknown";
...
export const BASE_REQUIREMENTS: Record<
	string,
...
export function computeTaskRequirements(
	...
	if (unitType === "execute-task" && metadata) {
		if (
			metadata.tags?.some((t) =>
				/^(docs?|readme|comment|config|typo|rename)$/i.test(t),
			)
		) {
			return { ...base, instruction: 0.9, coding: 0.3, speed: 0.7 };
		}
		if (
			metadata.complexityKeywords?.some(
				(k) => k === "concurrency" || k === "compatibility",
			)
		) {
		...
		if (
			metadata.complexityKeywords?.some(
				(k) => k === "migration" || k === "architecture",
			)
		) {
			return { ...base, reasoning: 0.9, coding: 0.8 };
		}
		if (
			(metadata.fileCount ?? 0) >= 6 ||
...
export function scoreEligibleModels(
	...
	modelId: string;
	score: number;
	confidence: CapabilityProfileConfidence;
}> {
	...
	scored.sort((a, b) => {
		...
		const confDiff =
			PROFILE_CONFIDENCE_ORDINAL[b.confidence] -
			PROFILE_CONFIDENCE_ORDINAL[a.confidence];
...
export function getEligibleModels(
	...
	if (explicitModel) {
		...
		const match = availableModelIds.find(
			(id) => bareModelId(id) === bareExplicit,
		);
	...
	const preferred = findPreferredModelForTier(
		tier,
		availableModelIds,
		preferredModelId,
	);
...
export function resolveModelForComplexity(
	...
	if (tierOrdinal(requestedTier) >= tierOrdinal(configuredTier)) {
		...
		return {
			...
			fallbacks: crossProviderEquivalent
				? [
						...phaseConfig.fallbacks.filter(
							(f) => f !== crossProviderEquivalent,
						),
						configuredPrimary,
					]
	...
	if (
		routingConfig.capability_routing !== false &&
		eligible.length > 1 &&
		unitType
	) {
		...
		if (winner) {
			...
			const rejected = scored
				...
				.map((s) => ({
					modelId: s.modelId,
					reason:
						profiled.length > 0 && s.confidence === "unknown"
...
export function resolveModelForTier(
	...
	routingConfigOrCrossProvider:
		...
		| boolean = defaultRoutingConfig(),
	crossProvider?: boolean,
	preferredModelId?: string,
): string {
	const routingConfig =
		typeof routingConfigOrCrossProvider === "boolean"
			? defaultRoutingConfig()
			: { ...defaultRoutingConfig(), ...routingConfigOrCrossProvider };
	const allowCrossProvider =
		typeof routingConfigOrCrossProvider === "boolean"
			? routingConfigOrCrossProvider
	...
	if (availableModelIds.length === 0) {
		if (preferredModelId) {
			return normalizeResolvedTierModelId(
				preferredModelId,
				tier,
				routingConfig,
			);
	...
	const resolved = findModelForTier(
		tier,
		routingConfig,
		availableModelIds,
		allowCrossProvider,
	...
	const providerLocalFallback =
		findAvailableModelId(preferredModelId, availableModelIds) ??
		availableModelIds[0] ??
		canonicalModelForTier(tier);
	return normalizeResolvedTierModelId(
		providerLocalFallback,
		tier,
		routingConfig,
	);
}
...
function findAvailableModelId(
	...
	if (preferredProvider) {
		const providerMatch = availableModelIds.find(
			(id) =>
				modelProvider(id)?.toLowerCase() === preferredProvider &&
				bareModelId(id) === preferredBare,
		);
...
export function isToolCompatibleWithProvider(
	...
	if (
		compat.schemaFeatures?.some((f) =>
			providerCaps.unsupportedSchemaFeatures.includes(f),
		)
	) {
		return false;
	}
...
export function filterToolsForProvider(
	...
	if (
		provider &&
		GROQ_PROVIDER_IDS.has(provider) &&
		compatible.length > GROQ_MAX_TOOLS
	) {
...
export function adjustToolSet(
	...
	const { compatible, filtered } = filterToolsForProvider(
		activeToolNames,
		selectedModelApi,
		provider,
	);
