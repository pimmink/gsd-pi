...
export const TOP_LEVEL_SUBCOMMANDS: readonly GsdCommandDefinition[] = [
	...
	{
		cmd: "pause",
		desc: "Pause auto-mode (preserves state, /gsd auto to resume)",
	},
	...
	{
		cmd: "brief",
		desc: "Generate a visual HTML brief: diagram, plan, diff review, recap, table, or slides",
	},
	...
	{
		cmd: "verdict",
		desc: "Override an unadopted compatibility milestone validation verdict",
	},
	...
	{
		cmd: "report",
		desc: "Generate all HTML reports and open the reports index",
	},
	{ cmd: "export", desc: "Alias for /gsd report" },
	{ cmd: "cleanup", desc: "Remove merged branches or snapshots" },
	{
		cmd: "closeout",
		desc: "Recover failed git closeout actions (status, retry, resolve)",
	},
	{
		cmd: "rebuild",
		desc: "Rebuild markdown projections from the canonical DB",
	},
	{
		cmd: "db",
		desc: "Database maintenance — restore-backup lists and restores verified pre-migration backups",
	},
	...
	{
		cmd: "config",
		desc: "(deprecated) Set tool API keys — use /gsd keys instead",
	},
	{
		cmd: "keys",
	...
	{
		cmd: "copilot-models",
		desc: "Check the live GitHub Copilot model catalog for drift (network only if Copilot is configured)",
	},
	{ cmd: "logs", desc: "Browse activity logs, debug logs, and metrics" },
	{
		cmd: "usage",
		desc: "Current LLM context window usage and session token totals",
	},
	{
		cmd: "context",
		desc: "Context breakdown chart (skills, injections, history)",
	},
	{ cmd: "debug", desc: "Create and inspect persistent /gsd debug sessions" },
	{ cmd: "forensics", desc: "Examine execution logs" },
	{
		cmd: "init",
		desc: "Project init wizard — detect, configure, bootstrap .gsd/",
	},
	{
		cmd: "setup",
		desc: "Configuration hub: status + sub-routes (llm, model, search, remote, keys, prefs, onboarding)",
	},
	{
		cmd: "onboarding",
		desc: "Re-run the setup wizard  [--resume|--reset|--step <name>]",
	},
	{
		cmd: "migrate",
		desc: "Migrate a v1 .planning directory to DB-backed .gsd with backup + audit",
	},
	...
	{
		cmd: "new-project",
		desc: "Bootstrap a new project (use --deep for staged project-level discovery)",
	},
	{
		cmd: "parallel",
	...
	{
		cmd: "upgrade",
		desc: "Alias for update; installs the latest @opengsd package",
	},
	{
		cmd: "start",
	...
	{
		cmd: "workflow",
		desc: "Custom workflow lifecycle (new, run, list, info, install, uninstall, validate, pause, resume) or run <name> directly",
	},
	{
		cmd: "codebase",
		desc: "Generate, refresh, and inspect the codebase map cache (.gsd/CODEBASE.md)",
	},
	{
		cmd: "ship",
	...
	{
		cmd: "scan",
		desc: "Rapid codebase assessment — lightweight alternative to full map (--focus tech|arch|quality|concerns|tech+arch)",
	},
	{
		cmd: "language",
		desc: "Set or clear the global response language (e.g. /gsd language Chinese)",
	},
	{
		cmd: "worktree",
	...
	{
		cmd: "eval-review",
		desc: "Audit a slice's AI evaluation strategy and write a scored EVAL-REVIEW.md (--force, --show)",
	},
...
const NESTED_COMPLETIONS: CompletionMap = {
	brief: VISUAL_BRIEF_MODES.map((mode) => ({
		cmd: mode.mode,
	...
	discuss: [
		{
			cmd: "--milestone",
			desc: "Target a specific milestone by ID (e.g. --milestone M014)",
		},
		{
			cmd: "--slice",
			desc: "Target a specific slice by ID (e.g. --slice M014/S03)",
		},
	],
	auto: [
	...
	setup: [
		...
		{
			cmd: "remote",
			desc: "Configure remote integrations (Discord/Slack/Telegram)",
		},
		{ cmd: "keys", desc: "Manage API keys (alias for /gsd keys)" },
		{ cmd: "prefs", desc: "Global preferences wizard (alias for /gsd prefs)" },
		{
			cmd: "onboarding",
			desc: "Run the full onboarding wizard (alias for /gsd onboarding)",
		},
	],
	onboarding: [
		...
		{
			cmd: "--reset",
			desc: "Reset onboarding state and start over (does not clear API keys)",
		},
		{
			cmd: "--step",
			desc: "Run a single step: llm|model|search|remote|tool-keys|prefs|skills|doctor|project",
		},
	],
	notifications: [
	...
	"copilot-models": [
		{
			cmd: "sync",
			desc: "Fetch and diff the live GitHub Copilot model catalog",
		},
		{
			cmd: "changes",
			desc: "Show the most recent drift summary for the live catalog",
		},
		...
		{
			cmd: "why <model>",
			desc: "Explain a model's tier, confidence, and pricing",
		},
	],
	debug: [
		...
		{
			cmd: "--diagnose",
			desc: "Inspect malformed artifacts and session health",
		},
	],
	keys: [
	...
	cleanup: [
		...
		{
			cmd: "projects",
			desc: "Audit orphaned ~/.gsd/projects/ state directories",
		},
		{
			cmd: "projects --fix",
	...
	closeout: [
		...
		{
			cmd: "resolve",
			desc: "Mark closeout resolved after the worktree is clean",
		},
	],
	rebuild: [
		{
		...
		{
			cmd: "database",
			desc: "Reserved for DB-native rebuilds; does not import markdown",
		},
	],
	db: [
		{
			cmd: "restore-backup",
			desc: "List or restore a verified pre-migration database backup (destructive; requires --consent)",
		},
	],
	knowledge: [
	...
	mcp: [
		...
		{
			cmd: "init",
			desc: "Write .mcp.json for the local GSD workflow MCP server",
		},
	],
	doctor: [
	...
	verdict: [
		...
		{
			cmd: "needs-attention",
			desc: "Override an unadopted verdict to needs-attention (requires --rationale)",
		},
		{
			cmd: "needs-remediation",
			desc: "Override an unadopted verdict to needs-remediation (requires --rationale)",
		},
	],
	rate: [
	...
	usage: [{ cmd: "--json", desc: "Machine-readable JSON output" }],
	context: [
	...
	scan: [
		...
		{
			cmd: "--focus arch",
			desc: "Architecture patterns and directory structure",
		},
	...
	worktree: [
		...
		{
			cmd: "remove",
			desc: "Remove a worktree (--force to skip safety checks)",
		},
	],
};
...
export function getGsdArgumentCompletions(prefix: string) {
	...
	if (
		...
		try {
			const defsDir = join(
				resolveProjectRootForCompletion(process.cwd()),
				".gsd",
				"workflow-defs",
			);
