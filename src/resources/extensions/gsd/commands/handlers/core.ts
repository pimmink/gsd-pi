...
export async function handleStatus(
	...
	const result = await ctx.ui.custom<boolean>(
		(tui, theme, _kb, done) =>
			new GSDDashboardOverlay(tui, theme, () => done(true)),
		{
			overlay: true,
			overlayOptions: {
				width: "90%",
...
export async function handleVisualize(
	...
	const result = await ctx.ui.custom<boolean>(
		(tui, theme, _kb, done) =>
			new GSDVisualizerOverlay(tui, theme, () => done(true)),
		{
			overlay: true,
			overlayOptions: {
				width: "80%",
	...
	if (result === undefined) {
		ctx.ui.notify(
			"Visualizer requires an interactive terminal. Use /gsd status for a text-based overview.",
			"warning",
		);
	}
}
...
export async function handleBrief(
	...
	if (!pi?.sendUserMessage) {
		ctx.ui.notify(
			"Visual brief generation is unavailable in this context.",
			"warning",
		);
		return;
	}
...
function resolveGsdVersion(): string | undefined {
	...
	try {
		const pkg = briefRequire("../../../../../../package.json") as {
			version?: unknown;
		};
...
export async function handleSetup(
	...
	const { detectProjectState, hasGlobalSetup } = await import(
		"../../detection.js"
	);
	const { isOnboardingComplete, readOnboardingRecord } = await import(
		"../../onboarding-state.js"
	);
	...
	if (args === "keys") {
		ctx.ui.notify(
			"Tip: /gsd keys is the canonical command for API key management.",
			"info",
		);
	...
	statusLines.push(
		`  Global preferences: ${globalConfigured ? "configured" : "not set"}`,
	);
	statusLines.push(`  Project state:      ${detection.state}`);
	if (detection.projectSignals.primaryLanguage) {
		statusLines.push(
			`  Detected:           ${detection.projectSignals.primaryLanguage}`,
		);
	}
...
function sortModelsForSelection(
	models: Model<any>[],
...
async function handleModel(
	...
	if (availableModels.length === 0) {
		ctx.ui.notify(
			"No available models found. Check provider auth and model discovery.",
			"warning",
		);
		return;
	}
	...
	if (!trimmed) {
		if (!ctx.hasUI) {
			const current = ctx.model
				...
				: "(none)";
			ctx.ui.notify(
				`Current model: ${current}\nUsage: /gsd model <provider/model|model-id>`,
				"info",
			);
			return;
		}

		targetModel = await selectModelByProvider(
	...
	if (!targetModel) {
		ctx.ui.notify(
			`Model "${trimmed}" not found. Use /gsd model with an exact provider/model or a unique model ID.`,
			"warning",
		);
		return;
	}
	...
	if (!ok) {
		ctx.ui.notify(
			`No API key for ${targetModel.provider}/${targetModel.id}`,
			"warning",
		);
		return;
	}
	...
	const pinNote = pinSession
		? ""
		: " (default updated; per-phase prefs still apply in auto)";
	ctx.ui.notify(
		`Model: ${targetModel.provider}/${targetModel.id}${pinNote}`,
		"info",
	);
}
...
export async function handleCoreCommand(
	...
	if (trimmed === "context" || trimmed.startsWith("context ")) {
		...
		await handleContext(
			trimmed.replace(/^context\s*/, "").trim(),
			ctx,
			projectRoot(),
		);
		return true;
	...
	if (trimmed === "show-config") {
		const { GSDConfigOverlay, formatConfigText } = await import(
			"../../config-overlay.js"
		);
		const basePath = projectRoot();
		...
		const availableModelIds = modelIdsForProfileResolution(
			ctx.modelRegistry,
			anchorProvider,
			disabledProviders,
		);
		const configOptions = {
			...
			...(availableModelIds && availableModelIds.length > 0
				...
				: {}),
			...(ctx.model
				? { preferredModelId: `${ctx.model.provider}/${ctx.model.id}` }
				: {}),
		};
		const result = await ctx.ui.custom<boolean>(
			(tui, theme, _kb, done) =>
				new GSDConfigOverlay(tui, theme, () => done(true), configOptions),
			{
				overlay: true,
				overlayOptions: {
					width: "65%",
				...
				milestone.status === "complete"
					...
					: milestone.status === "active"
						...
						: milestone.status === "parked"
							...
							: "○";
			lines.push(
				`  ${icon} ${milestone.id}: ${milestone.title} (${milestone.status})`,
			);
		}
	}
...
function buildWorkspaceRepoStatusBlock(basePath: string): string {
	let registry;
	try {
		registry = createRepositoryRegistryFromPreferences(
			basePath,
	...
	const childRepos = registry.repositories.filter(
		(repo) => repo.id !== "project",
	);
