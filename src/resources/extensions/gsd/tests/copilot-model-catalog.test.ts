...
test("fetchGitHubCopilotModels fetches GET /models and sanitizes data", async () => {
	...
	assert.deepEqual(
		...
		["mai-code-1.1-flash", "claude-sonnet-5"],
	);
...
test("diffCatalogSnapshots reports adds, removals, and changes", () => {
	...
	assert.deepEqual(
		diff.added.map((model) => model.id),
		["gpt-5.4"],
	);
	assert.deepEqual(
		diff.removed.map((model) => model.id),
		["claude-sonnet-5"],
	);
	assert.deepEqual(
		diff.changed.map((model) => model.id),
		[],
	);
});
...
test("computeCatalogRegistrationCandidates subtracts the effective local catalog", () => {
	...
	assert.equal(candidates.every((candidate) => candidate.quarantineReason.length > 0), true);
});

test("registerCopilotModelsInOverlay quarantines remote-only models instead of writing fabricated metadata", () => {
	...
	assert.deepEqual(plan.persisted, []);
	assert.deepEqual(
		plan.quarantined.map((model) => model.id),
		["mai-code-1.1-flash"],
	);
	assert.deepEqual(plan.overlay.models, {});
});
