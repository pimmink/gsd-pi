 ...
 */
export type GitHubCopilotProvider =
	| "github-copilot"
	| "openai"
	| "anthropic"
	| "google"
...
export function sanitizeGitHubCopilotModels(
	payload: { data?: unknown[] | null } | null | undefined,
): CopilotModelRecord[] {
...
export async function fetchGitHubCopilotModels(
	options: FetchCopilotModelsOptions,
): Promise<FetchCopilotModelsResult> {
...
export function applyLastKnownGood<T>(
	previous: T,
	next: { ok: boolean; error?: string; snapshot: T | null },
): T {
		...
		.filter(([id, model]) => {
			...
			return (
				...
				(previousModel.name !== model.name ||
					previousModel.tool_call !== model.tool_call)
			);
		})
		.map(([, model]) => model);
